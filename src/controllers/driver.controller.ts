...
export const getLoggedInDriver = async (req: Request, res: Response) => {
  try {
    ...
    const driver = await Driver.findOne({
      $or: [{ email: application.email }, { name: application.fullName }],
    });
    if (!driver) {
...
export const updateDriverStatus = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[DriverStatusUpdate] ID: ${id}, isOnline: ${isOnline}, currentLocation:`,
    ...
    console.log(
      `[DriverStatusUpdate] Successfully updated driver ${id}:`,
      JSON.stringify(driver)
    );
    res.json(driver);
  } catch (error) {
    console.error(`[DriverStatusUpdate] Error updating driver:`, error);
    res.status(500).json({ error: "Server error" });
  }
};
